...
@Component
public class ContractsDataHelper {
    @Autowired
    ...
    @Autowired
    private ReferenceDataPointsService referenceDataPointsService;

    private MarsEntity marsEntity;
    private Integer propertyId;
    private Integer managementCompanyId;

    public ContractsDataHelper(){
        try {
            marsEntity= GsonMapper.readJSONFileForAllCollections("testdata/ContractData.json");
        } catch (IOException e) {
            e.printStackTrace();
        }
        managementCompanyId=1005;
        propertyId=1905;
    }
    ...
    public void setupInventoryConfigData(){
        inventoryConfigService.deleteAllByPropertyId(propertyId);
        inventoryConfigService.deleteAllByManagementId(managementCompanyId);
        inventoryConfigService.createAll(marsEntity.getInventoryConfigs());
    }

    public void setupContractsData(){
        contractService.deleteAllByPropertyId(propertyId);
        contractService.createAll(marsEntity.getContracts());
    }

    public void createReferenceDataPointData(){
        referenceDataPointsService.deleteAll(propertyId);
        referenceDataPointsService.createAll(marsEntity.getReferenceDataPoints());
    }

    public void deleteContractsData(){
        contractService.deleteAllByPropertyId(propertyId);
    }
